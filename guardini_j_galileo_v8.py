# neuralprophet conda
# =====================================================================
# Forecasting Materiali Guardini - v8 (basata su v6, versione cliente)
# Correzioni v8:
#  1) Era-diventa: sostituzione TRANSITIVA dei codici (risolve le catene a più
#     passaggi; i vecchi codici intermedi non restano più nel forecast).
#  2) Classificazione: soglia ADI abbassata da 4.00 a 1.40 (standard SB = 1.32;
#     1.40 è un compromesso). I codici intermittenti/gappy vengono instradati al
#     metodo statistico SBA (forecast piatto e verosimile) invece che a
#     NeuralProphet, che li sovrastimava (causa principale del picco di luglio).
#  3) Anti-picco (residuo): rimossa la doppia stagionalità annuale
#     (yearly_seasonality=False, m12 resta esplicita) + guardrail che limita i
#     forecast al massimo storico del codice (× fattore configurabile), per i
#     codici frequenti che NeuralProphet può comunque sovrastimare.
# =====================================================================
...
st.divider()
with col_2:
    st.title('Forecasting Materiali v8 | :orange[NeuralProphet]')
...
if not path_venduto:
    st.warning('"VENDUTO_LUG_23_GIU_26.xlsx" non caricato, aprire sidebar e caricare file')
    st.stop()
...
def opt_model(df, periodi=12, use_optimized=False):
    ...
    if use_optimized and opt_data:
        ...
        # Default "Iron"
        # FIX v8: yearly_seasonality=False -> evita la DOPPIA stagionalità annuale.
        #   Il modello aggiunge già esplicitamente add_seasonality(name="m12",
        #   period=12) più sotto; lasciando anche la yearly='auto' di NeuralProphet
        #   la componente stagionale veniva contata due volte e, in modalità
        #   moltiplicativa su serie corte/rumorose, generava picchi sovrastimati
        #   (es. il picco anomalo di luglio segnalato dal cliente).
        # FIX v8: seasonality_reg alzato (0.1 -> 0.3) per regolarizzare meglio la
        #   stagionalità ed evitare overfitting del picco su poche stagioni.
        m_np = NeuralProphet(
            trend_global_local='local',
            season_global_local='local',
            seasonality_mode='multiplicative', # 'additive'
            yearly_seasonality=False,  # FIX v8: m12 è aggiunta esplicitamente sotto
            ...
            n_changepoints=3,
            trend_reg=0.2, # 1.0
            seasonality_reg=0.3,  # FIX v8: era 0.1
            epochs=60,
            loss_func='MSE', # Huber
...
# Creazione dataframe con continuità da era-diventa
df_con_sostituzione_era_diventa = df_venduto.copy()

# --- FIX v8: sostituzione TRANSITIVA dei codici era-diventa ------------------
# Nel file "era-diventa" possono esistere CATENE di sostituzione a più passaggi,
# es. 80124SGABB -> 80124SGABB2 -> 10124SGABB.
# La versione v6 applicava un solo "salto" (merge singolo Era->Diventa): così lo
# storico del codice più vecchio restava fermo sul codice intermedio (es.
# 80124SGABB2), che quindi NON spariva e compariva nel forecasting insieme al
# codice nuovo. Qui risolviamo la catena fino al codice FINALE.
def costruisci_mappa_transitiva(df_ed, max_iter=25):
    """Restituisce {codice_vecchio: codice_finale} risolvendo le catene
    era-diventa fino a convergenza, con protezione anti-ciclo."""
    mappa_diretta = dict(
        zip(df_ed['Era'].astype(str).str.strip(),
            df_ed['Diventa'].astype(str).str.strip())
    )
    mappa_finale = {}
    for codice_iniziale in mappa_diretta:
        ...
        for _ in range(max_iter):
            ...
            if successivo is None or successivo == corrente:
                break
            if successivo in visitati:  # ciclo: interrompe mantenendo l'ultimo
                break
            corrente = successivo
...
mappa_era_diventa = costruisci_mappa_transitiva(df_era_diventa)
n_catene = sum(1 for k, v in mappa_era_diventa.items()
               if mappa_era_diventa.get(v, v) != v or k != v)
df_con_sostituzione_era_diventa['Codice Articolo'] = (
    df_con_sostituzione_era_diventa['Codice Articolo'].map(
        lambda c: mappa_era_diventa.get(c, c))
)
st.caption(f"🔁 Era-diventa (v8): sostituzione transitiva applicata su "
           f"{len(mappa_era_diventa)} mappature (catene risolte fino al codice finale).")
# ----------------------------------------------------------------------------
...
# CLEANUP v8: pivot completo spostato in expander (tabella grezza, utile solo per verifica)
with st.expander('Mostra pivot venduto completo (codice × mese)'):
    st.dataframe(df_venduto_pivot)
...
with col2:
    ...
    with sb_col1:
        # FIX v8: soglia ADI abbassata da 4.00 (v6, fuori standard e troppo
        #   permissiva) a 1.40. La v6 mandava TUTTI i codici "frequenti" su
        #   NeuralProphet, anche quelli intermittenti/gappy che il modello
        #   sovrastimava (es. picco luglio di 10118FATBBAES). Con 1.40 i codici
        #   intermittenti vengono instradati al metodo statistico SBA (forecast
        #   piatto e verosimile). 1.40 è un compromesso tra lo standard
        #   Syntetos-Boylan (1.32) e un reroute meno esteso: risolve i codici
        #   segnalati mantenendo su NeuralProphet il grosso dei codici frequenti.
        ADI_LIMIT = st.number_input("Soglia ADI (standard SB = 1.32; qui 1.40)", value=1.40, step=0.01,
                                    help="Sopra questa soglia il codice è 'intermittente' → forecast statistico SBA. "
                                         "Standard Syntetos-Boylan = 1.32. La v6 usava 4.00 (troppo permissiva).")
    with sb_col2:
...
# CLEANUP v8: tabella dettaglio classificazione spostata in expander (grande;
# la sintesi utile è nei grafici matrice ADI/CV e nel riepilogo per classe sotto).
with st.expander('Mostra tabella dettaglio classificazione per codice (ADI, CV, classe)'):
...
with col_np:
    ...
    if not df_np_input.empty:
        ...
        # DISABILITATO in v8: l'ottimizzatore Optuna non è incluso in questa release
        # (richiede optuna + optimize_np.py). Il bottone è mostrato ma non cliccabile.
        st.sidebar.caption("⏸️ _Ottimizzatore non attivo in v8 (verrà reintrodotto in una release futura)._")
        if st.sidebar.button("🎯 Ottimizza NeuralProphet (Optuna)", disabled=True):
            try:
...
# --- FIX v8: GUARDRAIL ANTI-PICCO SUI FORECAST NEURALPROPHET ---
# Limita ogni previsione futura a (fattore × massimo mensile storico del codice),
# per evitare picchi irrealistici molto superiori a qualunque valore mai venduto
# (es. previsione di 16.000 pz a luglio a fronte di un massimo storico di 7.440).
st.markdown("**🛡️ Guardrail forecast (anti-picco) — FIX v8:**")
col_cap1, col_cap2 = st.columns(2)
with col_cap1:
    abilita_cap = st.toggle(
        "Limita i forecast al massimo storico (consigliato)", value=True,
        help="Evita previsioni NeuralProphet irrealistiche, molto superiori allo storico del singolo codice."
    )
with col_cap2:
    fattore_cap = st.number_input(
        "Fattore tetto (× max storico mensile)", min_value=1.0, max_value=5.0,
        value=1.5, step=0.1,
        help="Il forecast di ciascun codice viene limitato a: fattore × massimo mensile storico. "
             "1.0 = mai oltre il massimo mai venduto; 1.5 = lascia un margine del 50%."
    )
...
# --- FIX v8: GUARDRAIL ANTI-PICCO (cap sul massimo storico per codice) ---
def apply_forecast_cap(df_res, cap_map, fattore):
    """Limita le colonne yhat* a (fattore × max storico mensile del codice).
    Restituisce (df_capped, n_valori_limitati)."""
    df_out = df_res.copy()
    yhat_cols = [c for c in df_out.columns if c.startswith('yhat')]
    if 'ID' not in df_out.columns or not yhat_cols:
        return df_out, 0
    caps = df_out['ID'].map(lambda i: cap_map.get(i, np.inf)) * fattore
    n_capped = 0
    for col in yhat_cols:
        eccesso = df_out[col] > caps
        n_capped += int(eccesso.sum())
        df_out.loc[eccesso, col] = caps[eccesso]
    return df_out, n_capped

if abilita_cap:
    # max storico mensile per codice (df_venduto_pivot: index = Codice Articolo,
    # colonne = mesi). Applicato solo al forecast usato per gli output.
    max_storico_per_id = df_venduto_pivot.max(axis=1).to_dict()
    forecast_np, n_cap_fut = apply_forecast_cap(forecast_np, max_storico_per_id, fattore_cap)
    if n_cap_fut > 0:
        st.warning(
            f"🛡️ **Guardrail anti-picco (v8)**: {n_cap_fut} valori di forecast sono stati "
            f"limitati a **{fattore_cap:.1f}× il massimo storico mensile** del rispettivo codice, "
            f"per evitare previsioni irrealistiche."
        )
    else:
        st.info("🛡️ Guardrail anti-picco (v8) attivo: nessun forecast ha superato la soglia.")

st.success('Forecasting effettuato con successo (Iron-Clipping)!')

# CLEANUP v8: rimosse le tabelle grezze 'predicted' e 'forecast_np' duplicate
# (utili solo in debug). Restano i download e la tabella di forecast per orizzonte.

col_fcst_1, col_fcst_2 = st.columns(2)

with col_fcst_1:
    st.write('**Forecast NeuralProphet (storico + futuro):**')
    with st.expander('Mostra tabella completa forecast_np (grezza)'):
...
# CLEANUP v8: rimossi i display intermedi (df_galileo_pivot, df_galileo_sba e le
# viste parziali di df_galileo_unito). Restano solo la tabella finale + download.
